...
export const PurchaseCard: FC<Props> = ({ purchase }) => {
	...
	return (
		<article className='rounded-xl w-full flex gap-8 p-4 bg-lightGray border border-dark/20'>
				...
				<span className='flex items-center gap-2 h-1/2'>
					{purchase.status === 'shipped' ? (
						<>
							...
							{width >= 600 && <p className='text-sm'>Enviado (p/ {address})</p>}
							<button
								className='text-xs px-2 py-1 rounded border border-light text-light bg-green-600'
								onClick={() => handleClick()}
							>
